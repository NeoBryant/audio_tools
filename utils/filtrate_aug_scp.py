...
def parse_args():
    ...
    parser.add_argument("--in_dir", type=str, help="", 
                        default="/data3/pengjinghan/ffsvc2020_fbank_concat_cut_1200/data_list")
    ...
    parser.add_argument("--out_dir", type=str, help="", 
                        default="/data3/pengjinghan/ffsvc2020_fbank_concat_cut_1200/data_list_raw")
    
    args = parser.parse_args()
    return args

def is_augmentation(utt, filtrate_4xaug, filtrate_3xsp):
    """根据utt名判断是否为数据增强"""
...
def main(args):
    ...
    if os.path.isfile(feats_scp_path):
        with open(feats_scp_path, 'r') as rf, open(os.path.join(out_dir, "feats.scp"), 'w') as wf:
            for line in tqdm(rf.readlines()):
                utt = line.strip().split()[0]
                if not is_augmentation(utt, filtrate_4xaug, filtrate_3xsp):
                    raw_utt_set.add(utt)
                    wf.write(line)
    else:
    ...
    if os.path.isfile(utt2spk_path):
        ...
        cmd = f"./utt2spk_to_spk2utt.pl {out_utt2spk} > {out_spk2utt}"
        os.system(cmd)
    else:
